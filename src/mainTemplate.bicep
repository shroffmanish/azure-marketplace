...
var esSettings = {
  ...
  securityBootstrapPwd: ((!empty(securityBootstrapPassword)) ? securityBootstrapPassword : uniqueString(rg.id, deployment().name, securityAdminPassword))
...
var topologySettings = {
  ...
  logstashKeystorePwd: ((!empty(logstashKeystorePassword)) ? logstashKeystorePassword : uniqueString(rg.id, deployment().name, securityLogstashPassword))
  jumpbox: jumpbox
...
module shared 'partials/shared-resources.bicep' = {
  ...
  params: {
    location: deployment().location
    storageAccountName: 'elastic${uniqueString(rg.id, deployment().name)}'
...
module loadbalancer  'loadbalancers/external-lb-resources.bicep' = if (vNetNewOrExisting == 'new') {
  name: 'loadbalancer'
  params: {
    networkSettings: networkSettings
